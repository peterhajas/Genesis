...
{
    return [self initWithHost:theHost andPort:port withSSL:NO];
}

- (id)initWithHost:(NSString *)theHost andPort:(uint16_t)port withSSL:(BOOL)secure
{
    self = [self initWithHost:theHost andPort:port];
    if (self)
    {
